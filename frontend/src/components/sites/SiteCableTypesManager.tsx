...
export interface SiteCableTypesManagerProps {
  siteId: number;
  siteCode: string;
  siteName: string;
  onChanged?: () => void;
...
const SiteCableTypesManager: React.FC<SiteCableTypesManagerProps> = ({ siteId, siteCode: _siteCode, siteName: _siteName, onChanged }) => {
              ...
              {sortedCableTypes.map((ct) => (
                <div key={ct.id} className="flex items-start justify-between gap-3 px-3 py-2">
                  <div className="min-w-0">
                    ...
                    {ct.description ? <div className="text-xs text-muted-foreground whitespace-pre-wrap">{ct.description}</div> : null}
                  </div>

                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="sm" onClick={() => startEdit(ct)} disabled={working} title="Edit">
...
export default SiteCableTypesManager;
